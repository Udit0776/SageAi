...
export default function InterviewSession() {
  ...
  // Auto-start listening 2 seconds after AI is done speaking, unless manually paused or mic failed
  useEffect(() => {
    if (voiceMode && !isSpeaking && !isAnalyzing && !feedback && !isListening && !manuallyStopped && !hasMicError) {
      const timer = setTimeout(() => {
        startListening();
      }, 2000); // 2 second delay
      return () => clearTimeout(timer);
    }
  }, [voiceMode, isSpeaking, isAnalyzing, feedback, isListening, manuallyStopped, hasMicError, startListening]);
  ...
  const handleSubmit = async () => {
    ...
    // Stop listening and speaking immediately upon answer analysis
    stopListening();
    stopSpeaking();
    setManuallyStopped(false);
          ...
          <CardContent className="p-8 pt-0 space-y-6">
            ...
            {!feedback ? (
              <Button 
                ...
                className="w-auto mx-auto flex h-9 px-8 text-xs sm:text-sm font-bold shadow-xl shadow-primary/20 transition-all active:scale-[0.98]"
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <Button onClick={handleNext} className="w-auto mx-auto flex h-9 px-8 text-xs sm:text-sm font-bold" variant="secondary">
