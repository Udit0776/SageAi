 ...
 */
export function buildAdaptivePromptContext(weaknessProfile = [], userSkills = [], industry = "", last10Questions = []) {
  // If fewer than 3 assessments exist (new user), return default context
  // Wait, how do we know how many assessments exist?
  // We can calculate the total assessments count based on attempts or count of assessments.
  // Wait, if computeWeaknessProfile receives assessments and we pass them, or if we pass the count:
  // Let's check how many assessments were parsed. If we have less than 3 assessments, we return default context.
  // Let's pass a parameter or we can just count the unique assessment records.
  // Wait, if weaknessProfile is empty or we check the caller's assessments count:
  // Let's make buildAdaptivePromptContext check the weaknessProfile or take a third/fourth parameter.
  // Wait! Let's pass a fourth parameter: assessmentsCount (or check the length of assessments in the caller).
  // Wait, the signature requested is: Function buildAdaptivePromptContext(weaknessProfile, userSkills, industry)
  // But we can check if weaknessProfile is empty or check assessments length. Let's pass `assessmentsCount` or check `assessmentsCount`.
  // Wait, what if we just count how many assessments exist by looking at a custom property or passing it?
  // Let's look at the instruction:
  // "If fewer than 3 assessments exist (new user), return default context"
  // Let's make the function accept an optional parameter `assessmentsCount` or infer it.
  // Let's define the function as:
  // buildAdaptivePromptContext(weaknessProfile, userSkills, industry, last10Questions = [], assessmentsCount = 0)
  // That's perfect and extremely robust!
  
  const assessmentsCount = arguments[4] !== undefined ? arguments[4] : (weaknessProfile.length > 0 ? 3 : 0); 
  // Let's fall back to counting attempts as a proxy if assessmentsCount is not explicitly passed.
  // But we will pass it from action/interview.js!
  
  if (assessmentsCount < 3) {
    return ""; // Empty/default context will result in uniform/standard difficulty quiz.
  }
  ...
  const cleanLastQuestions = last10Questions.slice(0, 10).map(q => `"${q}"`).join(", ");
 ...
 Avoid repeating these recently asked questions: [${cleanLastQuestions}]`;
}
