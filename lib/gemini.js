...
async function runGeminiCascade(prompt, options) {
  ...
  const perModelTimeout = options.useSearch ? 25000 : 8000; // Search-grounded requests need more time
    ...
    for (const modelName of MODEL_NAMES) {
      ...
      for (let attempt = 1; attempt <= 2; attempt++) {
        try {
          ...
          result = await requestWithTimeout(apiCall, perModelTimeout);
          ...
          if (isTimeout) {
            console.warn(
              `[Key ${keyIndex + 1}] ${modelName} request timed out (>${perModelTimeout / 1000}s). Skipping model...`,
            );
            break;
          }
...
export async function getAIResponse(prompt, options = {}) {
  // Global timeout: 90s for search/PDF requests, 60s for standard requests
  const globalTimeout = (options.useSearch || options.pdfData) ? 90000 : 60000;
  const timeoutPromise = new Promise((_, reject) =>
    setTimeout(() => reject(new Error("AI is busy — all models timed out. Please try again in a moment.")), globalTimeout)
  );
